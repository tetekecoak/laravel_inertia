...
export default function Form({title ,category }) {
    ...
    const handleSave = (e) => {
        e.preventDefault();
        if (category) {
            patch(route('blog-categories.update',category))
        }
        else
        post(route('blog-categories.store'))
    }

    const breadcrumbs = [{link : route('blog-categories.index'),name : "Tag"},{link : null,name : title}]
