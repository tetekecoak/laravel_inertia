        ...
        <AuthenticatedLayout header={title} breadcrumbs={breadcrumbs}>
            <Head title={title} />
                <div className='flex flex-wrap justify-between mb-4'>
                    <div>
                        {usePermission('blog-tags.create') && <Link href={route('blog-tags.create')}><Button color='primary'> <HiPlus className="mr-2 h-5 w-5"/> <span>Add Tag</span> </Button></Link> }
                    </div>
                </div>
                            ...
                            {data.map((item) => (
                                <Table.Row key={item.id} className="bg-white dark:border-base-700 dark:bg-base-800">
                                    ...
                                    <Table.Cell className='flex space-x-2'>
                                        {usePermission('blog-tags.edit') && <Link href={route('blog-tags.edit',item.id)} className="hover:text-yellow-300  "><HiPencilAlt className='h-4 w-4'></HiPencilAlt></Link> }
                                        {usePermission('blog-tags.delete') && <div onClick={() => openModal(item.id)} className="hover:text-red-500"><HiTrash className='h-4 w-4'></HiTrash></div>  }
                                    </Table.Cell>
                                </Table.Row>
                            ))}
                        </Table.Body>
